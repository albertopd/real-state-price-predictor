...
def postcode_lookup_latlon(postcode: str):
    ...
    if postcode in grouped_df.index:
        lat, lon = grouped_df.loc[postcode]
...
def get_EPC_encoded(epc_score: str) -> int:
    epc_mapping = {
        "A+": 8,
        "A": 7,
        "B": 6,
...
def get_subtype_encoded(subtype: str) -> int:
    subtype_mapping = {
        "APARTMENT": 1,
        "HOUSE": 2,
        "FLAT_STUDIO": 3,
...
def get_province_encoded(province: str, postcode:str) -> int:
    province_mapping = {
        "BRUSSELS": 1,
        "LUXEMBOURG": 2,
        "ANTWERP": 3,
    ...
    province_from_postcode = check_province_from_postcode(postcode)
...
def get_type_encoded(property_type: str) -> int:
    type_mapping = {
        "APARTMENT": 1,
        "HOUSE": 2,
    }
...
def check_province_from_postcode(postcode: str) -> str:
...
def preprocess(data: PropertyInput) -> dict:
    ...
    lat, lon = postcode_lookup_latlon(preprocessed_data["postCode"])
    preprocessed_data["lat"] = lat
    preprocessed_data["lon"] = lon
    
      # Encode categorical variables \
    preprocessed_data["province_encoded"] = get_province_encoded(
        preprocessed_data["province"],preprocessed_data["postCode"]
    )
    preprocessed_data["type_encoded"] = get_type_encoded(preprocessed_data["type"])
    preprocessed_data["subtype_encoded"] = get_subtype_encoded(
        preprocessed_data["subtype"]
    )
    preprocessed_data["epcScore_encoded"] = get_EPC_encoded(
        preprocessed_data["epcScore"]
    )
